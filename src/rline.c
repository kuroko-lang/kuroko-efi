...
extern void move_cursor_x(int _x);
 ...
 */
static int COLOR_FG        = 0xF;
static int COLOR_BG        = 0x0;
static int COLOR_ALT_FG    = 0x7;
static int COLOR_ALT_BG    = 0x0;
static int COLOR_KEYWORD   = 0xb;
static int COLOR_STRING    = 0xa;
static int COLOR_COMMENT   = 0x6;
static int COLOR_TYPE      = 0xe;
static int COLOR_PRAGMA    = 0x4;
static int COLOR_NUMERAL   = 0xc;
static int COLOR_RED       = 0x4;
static int COLOR_GREEN     = 0x2;
static int COLOR_ESCAPE    = 0x3;
static int COLOR_SEARCH_FG = 0x0;
static int COLOR_SEARCH_BG = 0x6;
static int COLOR_ERROR_FG  = 0xf;
static int COLOR_ERROR_BG  = 0x4;
static int COLOR_BOLD      = 0xf;
static int COLOR_LINK      = 0x9;
 ...
 */
static int flag_to_color(int _flag) {
	int flag = _flag & 0xF;
	switch (flag) {
		case FLAG_KEYWORD:
 ...
 * Set colors
 */
static void set_colors(int fg, int bg) {
	set_attr((bg << 4) | fg);
}
 ...
 */
static void set_fg_color(int fg) {
	set_attr(fg);
}
...
static void render_line(void) {
	...
	while (i < line->actual) {
		...
		if (j >= offset) {
			...
			if (j - offset + c.display_width >= width - prompt_width_calc - 1) {
				/* We draw this with special colors so it isn't ambiguous */
				set_colors(COLOR_ALT_FG, COLOR_ALT_BG);

				/* If it's wide, draw ---> as needed */
				while (j - offset < width - prompt_width_calc - 2) {
					printf("-");
					j++;
				}
			...
			if (c.flags & FLAG_SELECT) {
				...
				last_color = color;
			} else if (last_color  == -1 || color != last_color) {
				set_fg_color(color);
				last_color = color;
			}
	...
	for (; j < width + offset - prompt_width - 1; ++j) {
		printf(" ");
	}
}
